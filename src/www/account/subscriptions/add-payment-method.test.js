...
describe('/account/subscriptions/add-payment-method', function () {
  describe('view', () => {
    it('should present the form for no stripe.js', async () => {
      ...
      const req = TestHelper.createRequest(`/account/subscriptions/add-payment-method?customerid=${user.customer.id}`)
      req.account = user.account
      req.session = user.session
      const result = await req.get()
      const doc = TestHelper.extractDoc(result.html)
      assert.strictEqual(doc.getElementById('stripe-v3'), undefined)
      assert.strictEqual(doc.getElementById('client-v3'), undefined)
    ...
    it('should present the form for stripe.js v3', async () => {
      ...
      assert.strictEqual(doc.getElementById('client-v3').tag, 'script')
  ...
  describe('submit', () => {
    ...
    it('should add payment information no stripe.js (screenshots)', async () => {
      ...
      req.screenshots = [
        ...
        { click: `/account/subscriptions/add-payment-method?customerid=${user.customer.id}` },
        { fill: '#form-nojs' }
      ]
      const result = await req.post()
      const doc = TestHelper.extractDoc(result.html)
