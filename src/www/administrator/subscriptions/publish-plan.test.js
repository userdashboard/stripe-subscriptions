...
describe('/administrator/subscriptions/publish-plan', function () {
  describe('exceptions', () => {
    ...
    it('should reject published plan', async () => {
      ...
      const req = TestHelper.createRequest(`/administrator/subscriptions/publish-plan?planid=${administrator.plan.id}`)
      req.account = administrator.account
      req.session = administrator.session
      let errorMessage
      try {
  ...
  describe('submit', () => {
    it('should publish plan (screenshots)', async () => {
      ...
      req.screenshots = [
        ...
        { click: `/administrator/subscriptions/publish-plan?planid=${administrator.plan.id}` },
        { fill: '#submit-form' }
      ]
