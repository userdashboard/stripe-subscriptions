...
describe('/api/administrator/subscriptions/subscription-charges', () => {
  describe('SubscriptionCharges#GET', () => {
    it('should limit charges on subscription to one page', async () => {
      ...
      const plan2 = await TestHelper.createPlan(administrator, {productid: product.id, published: true, amount: 2000, interval: 'day'})
      const plan3 = await TestHelper.createPlan(administrator, {productid: product.id, published: true, amount: 3000, interval: 'week'})
