...
describe(`/api/user/subscriptions/pay-invoice`, () => {
  describe('PayInvoice#PATCH', () => {
    it('should reject invalid invoiceid', async () => {
      const user = await TestHelper.createUser()
      await TestHelper.createCustomer(user, true)
      const req = TestHelper.createRequest(`/api/user/subscriptions/pay-invoice?invoiceid=invalid`, 'PATCH')
    ...
    it('should reject other account\'s invoice', async () => {
      ...
      const req = TestHelper.createRequest(`/api/user/subscriptions/pay-invoice?invoiceid=${user.invoice.id}`, 'GET')
    ...
    it('should reject paid invoice', async () => {
      ...
      const user = await TestHelper.createUser()
      await TestHelper.createSubscription(user, administrator.plan.id)
      const req = TestHelper.createRequest(`/api/user/subscriptions/pay-invoice?invoiceid=${user.invoice.id}`, 'PATCH')
      req.account = user.account
      req.session = user.session
      req.customer = user.customer
      req.body = {
    ...
    it('should reject forgiven invoice', async () => {
      ...
      await req.route.api.patch(req)
      const req2 = TestHelper.createRequest(`/api/user/subscriptions/pay-invoice?invoiceid=${user.invoice.id}`, 'PATCH')
    ...
    it('should reject other account\'s source', async () => {
      ...
      const req = TestHelper.createRequest(`/api/user/subscriptions/pay-invoice?invoiceid=${user2.invoice.id}`, 'GET')
      req.account = user2.account
      req.session = user2.session
      req.customer = user2.customer
      req.body = {
