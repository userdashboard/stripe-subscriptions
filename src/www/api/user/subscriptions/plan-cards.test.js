...
describe('/api/user/subscriptions/plan-cards', () => {
  describe('PlanCards#GET', () => {
    it('should limit cards to one page', async () => {
      ...
      await TestHelper.createSubscription(user, administrator.plan.id)
      await TestHelper.waitForNextItem(`subscription:charges:${user.subscription.id}`, null)
      const req = TestHelper.createRequest(`/api/user/subscriptions/plan-cards?planid=${administrator.plan.id}`, 'GET')
    ...
    it('should enforce specified offset', async () => {
      ...
      const req = TestHelper.createRequest(`/api/user/subscriptions/plan-cards?planid=${administrator.plan.id}&offset=${offset}`, 'GET')
      req.account = user.account
      req.session = user.session
      req.customer = user.customer
      const cardsNow = await req.route.api.get(req)
